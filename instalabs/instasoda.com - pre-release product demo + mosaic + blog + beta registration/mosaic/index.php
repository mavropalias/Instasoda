...
while($row = $result->fetch ()) {
	if($counter === 1){
		echo '<article class="item doubleX storyBox">
					<h1>Share your dating story:</h1>
					
					<p>
					    <textarea name="storyContent" id="storyContent" 
					    class="rte-zone">type your story here...</textarea>
					</p>
					<input type="text" class="storyNickname" placeholder="type your nickname here" />
					<input type="button" id="storySubmit" class="button" value="submit your story" />
					</article>';
	}
	else {
		if($row['admin_post']) $extraClass = "doubleXY";
		else $extraClass = "";
		
	  echo '<article class="item clickable '.$extraClass.'">'.
	  				'<h1><a>'.$row['title'].'</a></h1>'.
	  				'<cite>'.$row['date'].' by '.$row['author'].'</cite>'.
						'<span>'.
							'<p>'.$row['content'].'</p>'.
						'</span>'.
						'<footer><div class="fb-like" data-href="instasoda.com" data-send="false" data-layout="button_count" data-width="90" data-show-faces="false"></div></footer>'.
					'</article>';
	}
	$counter++;
}
