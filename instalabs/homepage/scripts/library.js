...
function initBoxes(){
  ...
  //log
  $('#log').prepend("<p>Adding "+boxRows+" rows and "+boxColumns+" columns.</p>");
}
...
function addBoxes(cols,rows){
  ...
  for(n = 0; n < rows; n++){
    ...
    $('#row'+n).css("top",(n*boxSize)+"px");
      ...
      for(m = 0; m < cols; m++){
        $('#row'+n).append("<div id='box"+ n + "-" + m +"' class='box box1'></div>");
        $('#box'+n+"-"+m).hide().css("left",(m*boxSize)+"px");
        
        //load random image
        rnd = Math.floor(Math.random() * (15 - 1) + 1);
        
        $('#box'+n+"-"+m).css("background-image",'url(faces/'+(rnd)+'.png)').delay(rnd*200).fadeTo(300, 0.8);
  ...
  //add mouseover event handler
    $('.box').mouseover(function(){
      var p = $(this).offset();
      $(this).fadeTo(0,1);
      $('#tooltip').css('left', (p.left - 1) + 'px').css('top', p.top - 1 + 'px').show();
    });
